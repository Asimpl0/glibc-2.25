...
// 将chunk的首地址转换为用户可以使用的mem的首地址，即去掉内存头后的位置
#define chunk2mem(p)   ((void*)((char*)(p) + 2*SIZE_SZ))
// 将用户可以使用的mem的首地址转换为chunk的首地址，即加上内存头后的位置
...
// 获得当前chunk的实际size，去掉了标记位
#define chunksize(p) (chunksize_nomask (p) & ~(SIZE_BITS))

/* Like chunksize, but do not mask SIZE_BITS.  */
// 获得当前chunk的实际size，没有去掉标记位
#define chunksize_nomask(p)         ((p)->mchunk_size)

/* Ptr to next physical malloc_chunk. */
// 获得当前chunk p的下一个chunk
#define next_chunk(p) ((mchunkptr) (((char *) (p)) + chunksize (p)))

/* Size of the chunk below P.  Only valid if prev_inuse (P).  */
// 获得当前chunk p的前一个chunk的size
#define prev_size(p) ((p)->mchunk_prev_size)

/* Set the size of the chunk below P.  Only valid if prev_inuse (P).  */
// 设置当前chunk p的前一个chunk的size
#define set_prev_size(p, sz) ((p)->mchunk_prev_size = (sz))

/* Ptr to previous physical malloc_chunk.  Only valid if prev_inuse (P).  */
// 获得当前chunk p的前一个chunk
#define prev_chunk(p) ((mchunkptr) (((char *) (p)) - prev_size (p)))

/* Treat space at ptr + offset as a chunk */
// 获得与当前chunk p相对偏移为s的chunk
#define chunk_at_offset(p, s)  ((mchunkptr) (((char *) (p)) + (s)))

/* extract p's inuse bit */
// 根据下一个chunk的pre_inuse来判断当前chunk是否空闲
#define inuse(p)							      \
  ((((mchunkptr) (((char *) (p)) + chunksize (p)))->mchunk_size) & PREV_INUSE)

/* set/clear chunk as being inuse without otherwise disturbing */
// 设置当前 chunk 的下一个chunk的pre_inuse，来标记当前chunk是否在使用
#define set_inuse(p)							      \
  ((mchunkptr) (((char *) (p)) + chunksize (p)))->mchunk_size |= PREV_INUSE
...
// 获得与当前chunk p的相对偏移为s的chunk的PREV_INUSE
#define inuse_bit_at_offset(p, s)					      \
  (((mchunkptr) (((char *) (p)) + (s)))->mchunk_size & PREV_INUSE)

// 设置与当前chunk p的相对偏移为s的chunk的PREV_INUSE
#define set_inuse_bit_at_offset(p, s)					      \
  (((mchunkptr) (((char *) (p)) + (s)))->mchunk_size |= PREV_INUSE)

// 清除与当前chunk p的相对偏移为s的chunk的PREV_INUSE
#define clear_inuse_bit_at_offset(p, s)					      \
  (((mchunkptr) (((char *) (p)) + (s)))->mchunk_size &= ~(PREV_INUSE))


/* Set size at head, without disturbing its use bit */
// 重新设置chunk p的size，并保留原来的标志位
#define set_head_size(p, s)  ((p)->mchunk_size = (((p)->mchunk_size & SIZE_BITS) | (s)))

/* Set size/use field */
// 设置当前chunk的size，不保留标志位
#define set_head(p, s)       ((p)->mchunk_size = (s))

/* Set size at footer (only when chunk is not in use) */
// 设置当前chunk p在下一个chunk中的prev_size，设置为s
...
/* bin中每一组都看做是一个malloc_chunk，仅使用其中的fd和bk，但是仍看作是一个完整的malloc_chunk
* 故第一个bin的起始实际是在malloc_state中mchunkptr top处，也即向前-16字节
* 第二个bin的起始就在第一个bin的位置，向前-16字节
* 所以bin初始化时，每一个bin的fd和bk都指向当前chunk的起始，也即-16字节的位置
*/

// 故此处bin at获得第i个bin时，由于每个bin有fd和bk，故占俩binptr，*2
// 然后第i个bin的起始实际要获得chunk的开头，故要减去chunk头也即16字节
// 此处offsetof (struct malloc_chunk, fd))即是获得fd的偏移，也即chunk头的大小
// bin_at返回的结果为mbinptr，也即malloc_chunk的起始地址，其中包含的fd和bk即为当前bin的俩指针
#define bin_at(m, i) \
  (mbinptr) (((char *) &((m)->bins[((i) - 1) * 2]))			      \
             - offsetof (struct malloc_chunk, fd))

/* analog of ++bin */
// (sizeof (mchunkptr) << 1即为16，bin的下一个bin的malloc_chunk起始即为当前bin+16
// next_bin返回的是下一个bin的malloc_chunk的起始地址
#define next_bin(b)  ((mbinptr) ((char *) (b) + (sizeof (mchunkptr) << 1)))

/* Reminders about list directionality within bins */
// 每一个bin都有fd和bk，fd指向当前bin的第一个chunk，bk指向当前bin的最后一个chunk
#define first(b)     ((b)->fd)
#define last(b)      ((b)->bk)
...
#define unlink(AV, P, BK, FD) {                                            \
    ...
    // 获得p的前一个chunk，链上的
    BK = P->bk;								      \
    // 如果p的下一个chunk的前一个不是p，或者p的前一个chunk的后一个不是p，异常
    if (__builtin_expect (FD->bk != P || BK->fd != P, 0))		      \
      malloc_printerr (check_action, "corrupted double-linked list", P, AV);  \
    else {								      \
        // 更新前后节点的指针，双向链表，不会出现空指针
        FD->bk = BK;							      \
        BK->fd = FD;							      \
        // 如果p的大小不在small bin中，且p的fd_nextsize存在，即为large bin中
        // large bin中还需要更新fd_nextsize和bk_nextsize
...
// 返回sz对应在small bin或者large bin中的索引
#define bin_index(sz) \
...
// bin中的第一个malloc_chunk即为unsorted bin
...
// 每一个bin对应map中的1bit，所以总数量/32即所需要的map的数量
// arena中包含多少个bin map
#define BINMAPSIZE       (NBINS / BITSPERMAP)

// 将bin的索引转换为其所在的map，用bin的索引/32即可
...
/*
* fastbin为单向链表，bin中的mfastbinptr指针直接指向malloc_chunk，fastbin数组中每一项不为malloc_chunk
*/
...
// 根据flags的第二位是否为0判断是否contiguous，为1时nocontiguous
#define noncontiguous(M)       (((M)->flags & NONCONTIGUOUS_BIT) != 0)
// 将flags的第二位设置为1 noncontiguous
#define set_noncontiguous(M)   ((M)->flags |= NONCONTIGUOUS_BIT)
// 将flags的第二位设置为0 contiguous
...
// 设置flags的ARENA_CORRUPTION_BIT为异常
...
struct malloc_state
{
  ...
  // 当前arena的flags,包含是否有fastbin,是否为contiguous,是否corrupt
  int flags;
...
_int_malloc (mstate av, size_t bytes)
{
  ...
  // 将待分配的bytes转换为实际的chunk大小，即nb
  checked_request2size (bytes, nb);

  /* There are no usable arenas.  Fall back to sysmalloc to get a chunk from
     mmap.  */
  
  // 如果arena为空，通过系统调用分配一块内存
  ...
  if (in_smallbin_range (nb))
    {
      ...
      // 也即如果bin->bk != bin 说明要么arena没有被初始化过，要么不为空，可以进行分配
      // 如果bin->bk == bin 说明当前bin中是空的，无法进行分配

      // small bin中分配为FIFO，分配时从尾部开始，估待分配的chunk即为victim
	...
	if (__glibc_unlikely (bck->fd != victim))
              ...
              // victim即将被使用，将victim下一个chunk的preuse置为1
  ...
  // 请求的大小在large中，在分配前，将fastbin中的chunk进行合并，为了避免碎片化问题
  else
    {
      // 获得该大小对应的large bin
...
static void malloc_consolidate(mstate av)
{
  ...
  */
  ...
  // 判断max_fast是否为0，为0说明arena还没被初始化过
  ...
  if (get_max_fast () != 0) {
    ...
    */
    ...
    // 获得fastbin中大小最大的chunk链的链首
    maxfb = &fastbin (av, NFASTBINS - 1);
    // 获得fastbin中大小最小的chunk链的链首
    fb = &fastbin (av, 0);
    // 遍历所有的fastbin，进行合并操作
    do {
      // 取出fb指向的fastbin中的chunk 为p
      p = atomic_exchange_acq (fb, NULL);
      // 取出的chunk不为空
      if (p != 0) {
    // 遍历当前fastbin上的所有chunk，进行合并操作
    ...
    // 获得p的前一个chunk是否在被使用
    // 如果为0说明前一个chunk没有被使用，则需与当前chunk进行合并
	  if (!prev_inuse(p)) {
      // 获得前一个chunk的大小
	    prevsize = prev_size (p);
      // 获得合并后的大小
    ...
    // 判断p的下一块chunk是否为top chunk，是的话会合并top chunk
	  if (nextchunk != av->top) {
      // 判断nextchunk是否空闲，根据nextchunk下一块chunk的preinuse判断
	    nextinuse = inuse_bit_at_offset(nextchunk, nextsize);

      // 判断是否空闲
	    if (!nextinuse) {
        // 空闲的话，size加上下一块chunk的大小
	      size += nextsize;
        // 从av中将nextchunk摘链，从fastbin中摘链
	      unlink(av, nextchunk, bck, fwd);
	    } else
        // 由于fastbin中的chunk不允许被合并，所以fastbin中chunk下一个chunk的preinuse都置为1
        // 此处我们要合并fastbin中的chunk了，故清除下一chunk的preinuse标志
	      clear_inuse_bit_at_offset(nextchunk, 0);

      // 获得unsorted bin的第一个chunk
	    first_unsorted = unsorted_bin->fd;
      // 将p插入到unsorted bin的链表开头
	    unsorted_bin->fd = p;
	    first_unsorted->bk = p;

      // 判断合并后的大小是否在small bin中
	    if (!in_smallbin_range (size)) {
        // 不在small bin中则初始化当前chunk的large指针
	      p->fd_nextsize = NULL;
	      p->bk_nextsize = NULL;
	    }

      // 更新当前chunk的size，同时将其前一块置为inuse
	    set_head(p, size | PREV_INUSE);
      // 更新p在unsorted bin中的指针
	    p->bk = unsorted_bin;
	    p->fd = first_unsorted;
      // 更新chunk下一块chunk的presize
	    set_foot(p, size);
	  }

    // p的下一块为top chunk，需要进行合并，不放入unsorted bin中
	  else {
      // 更新size加上下一块的size
	    size += nextsize;
      // 更新当前chunk的大小
	    set_head(p, size | PREV_INUSE);
      // 合并当前chunk和top后，更新top
	    av->top = p;
	  }
  // 遍历当前fastbin上所有chunk
	} while ( (p = nextp) != 0);
